...
class ProcessDataTask extends AsyncTask<Void, Void, ScanResult> {
    ...
    protected ScanResult doInBackground(Void... params) {
        ...
        if (mPicturePath != null) {
            return qrCodeView.processBitmapData(CodeUtil.getDecodeAbleBitmap(mPicturePath));
            ...
            if (CodeUtil.isDebug()) {
                CodeUtil.d("两次任务执行的时间间隔：" + (System.currentTimeMillis() - sLastStartTime));
            ...
            if (CodeUtil.isDebug()) {
                ...
                if (scanResult != null && !TextUtils.isEmpty(scanResult.result)) {
                    CodeUtil.d("识别成功时间为：" + time);
                } else {
                    CodeUtil.e("识别失败时间为：" + time);
                }
            }
